...
function modifier_tidehunter_blubber_lua:OnTakeDamage(kv)
	...
	if target ~= self:GetParent() or target:PassivesDisabled() or target:IsIllusion() or attacker:IsOther() or (attacker:GetOwnerEntity() == nil and attacker:GetPlayerOwnerID() == nil) or not self:GetAbility():IsCooldownReady() then return end
	...
	if self.damage_counter >= self:GetAbility():GetSpecialValueFor("damage_cleanse") then
		...
		target:EmitSound("Hero_Tidehunter.KrakenShell")
	end
end