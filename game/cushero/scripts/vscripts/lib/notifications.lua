...
Notifications = Notifications or class({})
...
function Notifications:Top(player, table, ping_location)
	if type(player) == "number" then
		player = PlayerResource:GetPlayer(player)
	end
	...
	if ping_location then
		PlayerResource:PingAtMinimap(player, ping_location)
	end
end

function Notifications:TopToAll(table, ping_location)
	if table.text ~= nil then
	...
	if ping_location then
		GameRules:PingAtMinimapAll(ping_location)
	end
end

function Notifications:TopToTeam(team, table, ping_location)
	if table.text ~= nil then
	...
	if ping_location then
		GameRules:PingAtMinimapTeam(ping_location)
	end
end


function Notifications:Bottom(player, table, ping_location)
	if type(player) == "number" then
		player = PlayerResource:GetPlayer(player)
	end
	if table.text ~= nil then
...
function Notifications:BottomToAll(table, ping_location)
	if table.text ~= nil then
...
function Notifications:BottomToTeam(team, table, ping_location)
	if table.text ~= nil then
